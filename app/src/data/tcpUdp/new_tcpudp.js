...
import newTemplate from 'plugins/kibana/data/tcpUdp/new_tcpudp.html';
import 'ui/anyrobot_ui/tm.pagination';
import 'plugins/kibana/data/ruleInput/rule_input_set';
import 'plugins/kibana/data/tcpUdp/save_tcpudp';
import 'ui/notify';
import tcpudpCom from 'plugins/kibana/data/components/tcpudp_com';

routes
.when('/data/tcpUdp/newTcpUdp', {
  template: newTemplate
});

uiModules
.get('apps/data', ['kibana/notify'])
.controller('newTcpUdp', function ($scope, $location, $http, locals, Notifier, $timeout,$translate,$translatePartialLoader) {
    ...
    $scope.dataAdd = function () {
      ...
      if (results === '') {
        let data = {
          ...
          //'tags': $scope.arLabel[0] === '' ? $scope.data.tags.split(/\uff0c|,/) : $scope.arLabel,
          'tags': $scope.arLabel,
        ...
        if ($scope.uplodeFlag) {
          ...
          $http.post('/etl/input/tcpUdp',data).success(function (res,status) {
            if (status === 200) {
          ...
          $http.put(`/etl/input/tcpUdp/${arDataId}`,data).success(function (res,status) {
            if (status === 200) {
  ...
  $scope.newRule = function () {
    ...
    locals.setObject('TcpArg',{
      data:$scope.data,id:$scope.dataId,port:$scope.data.port,Labels:$scope.arLabel,newR:true,ruleName:ruleName,type:'tcpudp'});//给解析规则获取相应的端口
  ...
  $scope.updetaRule = function () {
    ...
    $scope.dataId = $scope.dataId === undefined ? tcparg.id : $scope.dataId;
    locals.setObject('TcpArg',{
      data:$scope.data,id:$scope.dataId,port:$scope.data.port,Labels:$scope.arLabel,newR:false,ruleName:ruleName,type:'tcpudp'});
    $scope.jump('/data/ruleInput/ruleInputSet');
  };
    ...
    if (results === '') {
      ...
      $http.put(`/etl/input/tcpUdp/${arDataId}`,data).success(function (res,status) {
        if (status === 200) {
          ...
          $scope.jump('/data/tcpUdp/saveTcpudp');
        } else {
