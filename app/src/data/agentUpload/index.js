/*
by luo.chunxiang 2017.03.08
 */
import _ from 'lodash';
import $ from 'jquery';
import routes from 'ui/routes';
import uiModules from 'ui/modules';
import agentTemplate from 'plugins/kibana/data/agentUpload/template.html';
import 'ui/anyrobot_ui/tm.pagination';
import 'plugins/kibana/data/agentUpload/new_agent';
import 'ui/notify';

routes
.when('/data/agentUpload', {
...
.controller('agentCtrl', function ($scope, $location, $http, locals, Notifier,$translate,$translatePartialLoader,
  $modal,$timeout) {
 ...
 //配置分页基本参数
  ...
  var getRecordConfTotalItems = function () {
    $http.get(`/etl/input/agent/count?timestamp = ${new Date().getTime()}`).success(function (data) {
 ...
 //分页获取TCP/UDP
  var getDataLog = function (lim) {
    $http.get(`/etl/input/agent/list?start=${$scope.dataIndexStart}&limit=${lim}&timestamp = ${new Date().getTime()}`)
  ...
  $scope.newTcpUdpEdit = function (index) {
    ...
    $scope.jump('/data/agentUpload/newAgent');
  };
  ...
  $scope.curStateChange = function (item) {
    ...
    $http.put(`/etl/input/agent/${thisDataId}`,data).success(function () {
      notify.info(msg);
    }).error(function (res,status) {
      if (data.status) {
        data.status = 0;
  ...
  var newTcpUdpClear = function (index) {
    ...
    $http.delete(`/etl/input/agent/${deleteDataId}`).success(function () {
